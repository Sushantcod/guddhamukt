...
interface ToyCameraCheckboxProps {
  checked?: boolean;
...
export const ToyCameraCheckbox: React.FC<ToyCameraCheckboxProps> = ({
  checked = false,
  onChange,
  photoPreviewUrl,
  label = 'Click camera to snap photo',
  className,
}) => {
  return (
    <StyledWrapper className={className}>
      <div className="toy-camera-container">
          ...
          <div className="toy-camera-body">
            ...
            <div className="toy-camera-photo">
              {photoPreviewUrl ? (
                <img
                  src={photoPreviewUrl}
                  alt="Snapped evidence"
                  className="w-full h-[70%] object-cover rounded-[0.2em]"
                />
              ) : (
        ...
        {label && <p className="text-xs font-bold text-slate-600 mt-4 tracking-tight">{label}</p>}
      </div>
    </StyledWrapper>
  );
};
...
export default ToyCameraCheckbox;
